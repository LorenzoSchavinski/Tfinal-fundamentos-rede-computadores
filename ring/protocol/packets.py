...
def parse(datagram: bytes) -> dict:
    ...
    if prefixo in (P_DISCOVER, P_HELLO):
        ...
        return {
            "type": tipo,
            # Normaliza so o apelido (matching de membros); ip fica intacto.
            "apelido": partes[1].decode("ascii", errors="replace").strip().upper(),
    ...
    if prefixo == P_DATA:
        ...
        return {
            "type": "DATA",
            # Normaliza so apelidos de origem/destino (matching local); controle,
            # crc e message ficam intactos. O repasse usa raw, entao o fio nao muda.
            "origem": partes[1].decode("ascii", errors="replace").strip().upper(),
            "destino": partes[2].decode("ascii", errors="replace").strip().upper(),
